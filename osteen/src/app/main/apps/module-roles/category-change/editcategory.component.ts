...
export class EditCategoryDialog {
    ...
    ngOnInit(): void
    {
        ...
        this.editCategoryForm = this._formBuilder.group({
            ...
            category_name : [this.EditDataGet.category_name, Validators.required],
            category_id : [this.EditDataGet.category_id, Validators.required],
            enable : [this.EditDataGet.enable, Validators.required]
        });
    }
    editCategorySubmit() {
        this.submitted = true;
        // stop here if editCategoryForm is invalid
        if (this.editCategoryForm.invalid) {
            return;
        }
        this.userService.editCategoryChange(this.editCategoryForm.value)
            .pipe(first())
            .subscribe(
                data => {
