...
@Component({
  selector: 'add-governance-dialog',
  templateUrl: 'add-governance.component.html',
})
export class AddGovernanceDialog {
    direction = 'row';
    currentUser: any;
    ...
    constructor(
        public dialogRef: MatDialogRef<AddGovernanceDialog>,
    ...
    ngOnInit(): void
    {
        ...
        this.addGovernanceForm = this._formBuilder.group({
    ...
    addGovernanceSubmit() {
        this.submitted = true;
        return false;
        // stop here if addGovernanceForm is invalid
        if (this.addGovernanceForm.invalid) {
            return;
        }
       /*  this.userService.addPriorityChange(this.addGovernanceForm.value).pipe(first()).subscribe(
            (data:any) => {
