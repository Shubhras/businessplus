...
import { EditProcedureDialog} from 'app/main/apps/business-vitals/procedure-template/edit-procedure.component';
...
export class ProcedureComponent implements OnInit
{
    ...
    editProcedureOpen(element): void {
      const dialogRef = this.dialog.open(EditProcedureDialog, {
        // width: 'auto',
        panelClass: 'adding-dial',
        data: element
      });
    ...
    ProcedureGet() {
      ...
      this.userService.getPriorityChange(login_access_token,company_id).pipe(first()).subscribe(
        (data:any) => {
            /* this.ProcedureAllData =  data.data;
            this.ProcedureAllData.map((CATEGORY: any, index: number) => {
              CATEGORY.sr_no = index + 1;
            });
            const ELEMENT_DATA: PeriodicElement[] = this.ProcedureAllData; */
