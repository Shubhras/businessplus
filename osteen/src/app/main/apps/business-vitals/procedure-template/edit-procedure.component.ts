...
@Component({
    selector: 'edit-procedure-dialog',
    templateUrl: 'edit-procedure.component.html',
})
export class EditProcedureDialog {
    direction = 'row';
    currentUser: any;
    EditDataGet: any;
    ...
    constructor(
        public dialogRef: MatDialogRef<EditProcedureDialog>,
    ...
    ngOnInit(): void {
        ...
        // Reactive Form
        this.editProcedureForm = this._formBuilder.group({
    ...
    editProcedureSubmit() {
        this.submitted = true;
        // stop here if editProcedureForm is invalid
        if (this.editProcedureForm.invalid) {
            return;
        }
        this.userService.editPriorityChange(this.editProcedureForm.value).pipe(first()).subscribe(
            (data: any) => {
