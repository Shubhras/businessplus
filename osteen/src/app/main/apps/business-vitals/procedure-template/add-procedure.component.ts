...
export class AddProcedureDialog {
    ...
    constructor(
        public dialogRef: MatDialogRef<AddProcedureDialog>,
    ...
    ngOnInit(): void {
        ...
        this.addProcedureForm = this._formBuilder.group({
    ...
    addProcedureSubmit() {
        this.submitted = true;
        // stop here if addProcedureForm is invalid
        if (this.addProcedureForm.invalid) {
            return;
        }
        this.userService.addPriorityChange(this.addProcedureForm.value).pipe(first()).subscribe(
            (data: any) => {
