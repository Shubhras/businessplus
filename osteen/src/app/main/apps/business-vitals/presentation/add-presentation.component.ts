...
@Component({
  selector: 'add-presentation-dialog',
  templateUrl: 'add-presentation.component.html',
})
export class AddPresentationDialog {
    direction = 'row';
    currentUser: any;
    ...
    constructor(
        public dialogRef: MatDialogRef<AddPresentationDialog>,
    ...
    ngOnInit(): void
    {
        ...
        this.addPresentationForm = this._formBuilder.group({
    ...
    addPresentationSubmit() {
        this.submitted = true;
        // stop here if addPresentationForm is invalid
        if (this.addPresentationForm.invalid) {
            return;
        }
        /* this.userService.addPriorityChange(this.addPresentationForm.value).pipe(first()).subscribe(
            (data:any) => {
