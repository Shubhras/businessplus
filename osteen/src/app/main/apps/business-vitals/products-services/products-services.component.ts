...
import { AddProductsServicesDialog} from 'app/main/apps/business-vitals/products-services/add-products-services.component';
import { EditProductsServicesDialog} from 'app/main/apps/business-vitals/products-services/edit-products-services.component';
...
export class ProductsServicesComponent implements OnInit
{
    ...
    ngOnInit(): void
    {
        ...
        this.ProductsServicesGet();
    }
    addProductsServicesOpen(): void {
        const dialogRef = this.dialog.open(AddProductsServicesDialog, {
    ...
    editProductsServicesOpen(element): void {
      const dialogRef = this.dialog.open(EditProductsServicesDialog, {
        // width: 'auto',
        panelClass: 'adding-dial',
        data: element
      });
    ...
    ProductsServicesGet() {
      ...
      this.userService.getPriorityChange(login_access_token,company_id).pipe(first()).subscribe(
        (data:any) => {
            /* this.ProductsServicesAllData =  data.data;
            this.ProductsServicesAllData.map((CATEGORY: any, index: number) => {
              CATEGORY.sr_no = index + 1;
            });
            const ELEMENT_DATA: PeriodicElement[] = this.ProductsServicesAllData; */
