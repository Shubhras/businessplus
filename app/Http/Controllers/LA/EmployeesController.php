<?php
/**
 * Controller genrated using PrimaPlus
 * Help: http://PrimaPlus.com
 */
...
use Dwij\PrimaPlus\Models\Module;
use Dwij\PrimaPlus\Models\ModuleFields;

use Dwij\PrimaPlus\Helpers\LAHelper;
...
class EmployeesController extends Controller
{
	...
	public function __construct() {
		...
		if(\Dwij\PrimaPlus\Helpers\LAHelper::laravel_ver() == 5.3) {
	...
	public function index()
	{
		...
		if(Module::hasAccess($module->id)) {
            ...
            return redirect(config('PrimaPlus.adminRoute')."/");
        }
	}
		...
		if(Module::hasAccess("Employees", "create")) {
			...
			if(env('MAIL_USERNAME') != null && env('MAIL_USERNAME') != "null" && env('MAIL_USERNAME') != "") {
				...
				Mail::send('emails.send_login_cred', ['user' => $user, 'password' => $password], function ($m) use ($user) {
					...
					$m->to($user->email, $user->name)->subject('Welcome to Prima Plus!');
				});
			} else {
				Log::info("User created: username: ".$user->email." Password: ".$password);
			}
			
			return redirect()->route(config('PrimaPlus.adminRoute') . '.employees.index');
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('PrimaPlus.adminRoute') . '/employees/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Employees", "edit")) {
					$output .= '<a href="'.url(config('PrimaPlus.adminRoute') . '/employees/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Module::hasAccess("Employees", "delete")) {
					$output .= Form::open(['route' => [config('PrimaPlus.adminRoute') . '.employees.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
	...
	public function change_password($id, Request $request) {
		...
		if ($validator->fails()) {
			return \Redirect::to(config('PrimaPlus.adminRoute') . '/employees/'.$id)->withErrors($validator);
		}
		...
		if(env('MAIL_USERNAME') != null && env('MAIL_USERNAME') != "null" && env('MAIL_USERNAME') != "") {
			...
			Mail::send('emails.send_login_cred_change', ['user' => $user, 'password' => $request->password], function ($m) use ($user) {
				...
				$m->to($user->email, $user->name)->subject('PrimaPlus - Login Credentials change');
			});
		} else {
			Log::info("User change_password: username: ".$user->email." Password: ".$request->password);
		}
		
		return redirect(config('PrimaPlus.adminRoute') . '/employees/'.$id.'#tab-account-settings');
	}
}
